	...
	if 	curItem then
		...
		for i,v in ipairs(self.m_sNumGroup) do
			if 	v.index_h == curItem.index_h and v.index_v == curItem.index_v then
				...
				print("curItem.index_h---->",curItem.index_h)
				print("curItem.index_v---->",curItem.index_v)

				print("i-------->",i)
			
				print("self.m_sNumGroup[i].num",self.m_sNumGroup[i].num)
...
function CDGameNumDestory:onResetItemPos( )
	local curIndex = 1
	for i,v in ipairs(self.m_sNumGroup) do
		if 	i%8 == 1 and i ~= 1 then
			curIndex = curIndex+1
		end
		v.num = self:insRandomNum(curIndex)
